...
function DoctorDashboard() {
  ...
  const [pharmacyRooms, setPharmacyRooms] = useState([]);
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    ...
    const fetchRooms = async () => {
      try {
        const token = localStorage.getItem('token');
        const rooms = await axios.get('http://localhost:3002/roomsToJoin', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        setPharmacyRooms(rooms.data);
      } catch (error) {
        console.error('Error fetching doctors:', error);
      }
    };
  ...
  const handleRoomChatClick = async (e, roomId) => {
    alert(roomId);
    e.stopPropagation(); // Prevent the event from propagating to the parent elements
    navigate(`/chatPharmacy/${roomId}`);
  }
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-end">
        <div style={{ position: 'fixed', bottom: '20px', right: '300px', zIndex: '1000' }}>
          <div style={{ position: 'fixed', bottom: '20px', right: '300px', zIndex: '1000' }}>
            <Dropdown drop="up">
              <Dropdown.Toggle variant="secondary" id="dropdown-basic">
                Reply to Pharmacists
              </Dropdown.Toggle>

              <Dropdown.Menu>
                {pharmacyRooms.map((room, index) => (
                  <Dropdown.Item key={room._id}>
                    <div className="d-flex align-items-center justify-content-between">
                      {index > 0 && <span>, </span>}
                      Room{index}
                      <span style={{ marginLeft: '10px' }}>
                        ...
                        <FaComment
                          style={{ marginRight: '10px', cursor: 'pointer' }}
                          onClick={(e) => handleRoomChatClick(e, room._id)}
                        />
                      </span>
                    </div>
                  </Dropdown.Item>
